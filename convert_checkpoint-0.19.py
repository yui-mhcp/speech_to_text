...
def convert_openai_whisper_encoder(model_metadata: dict, model_params: dict, quant_algo: str = None,):
    weights = {}
    ...
    weights['transformer.conv1.bias'] = model_params['encoder.conv1.bias'].contiguous()
    weights['transformer.conv2.weight'] = torch.unsqueeze(
        model_params['encoder.conv2.weight'], -1).contiguous()
    weights['transformer.conv2.bias'] = model_params['encoder.conv2.bias'].contiguous()

    # Encoder conv needs to run in fp32 on Volta/Turing
    major, minor = torch.cuda.get_device_capability()
    if not major >= 8:
        weights['transformer.conv1.weight'] = weights['transformer.conv1.weight'].float()
        weights['transformer.conv1.bias'] = weights['transformer.conv1.bias'].float()
        weights['transformer.conv2.weight'] = weights['transformer.conv2.weight'].float()
        weights['transformer.conv2.bias'] = weights['transformer.conv2.bias'].float()
    ...
    weights['transformer.ln_f.bias'] = model_params['encoder.ln_post.bias'].contiguous()

    return weight_only_quantize_dict(weights, quant_algo=quant_algo, plugin=True)


def convert_openai_whisper_decoder(model_metadata: dict, model_params: dict, quant_algo: str = None):
    weights = {}
    ...
    for i in range(model_metadata['n_text_layer']):
        ...
        t = torch.cat([
            ...
            model_params['decoder.blocks.' + str(i) + '.attn.value.weight']
                      ...
                      dim=0)
        dst = weights[f'{trtllm_layer_name_prefix}.self_attention.qkv.weight'] = t

        t = model_params['decoder.blocks.' + str(i) +
                         '.attn.out.weight'].contiguous()
        dst = weights[f'{trtllm_layer_name_prefix}.self_attention.dense.weight'] = t
        ...
        t = torch.cat([
            ...
            model_params['decoder.blocks.' + str(i) +
                      ...
                      dim=0)
        dst = weights[f'{trtllm_layer_name_prefix}.cross_attention.qkv.weight'] = t

        t = model_params['decoder.blocks.' + str(i) +
                         '.cross_attn.out.weight'].contiguous()
        dst = weights[f'{trtllm_layer_name_prefix}.cross_attention.dense.weight'] = t
...
def convert_hf_whisper_encoder(model_metadata: dict, model_params: dict, quant_algo: str = None,):
    weights = {}
    ...
    for i in range(model_metadata['n_audio_layer']):
        ...
        weights[
            f'{trtllm_layer_name_prefix}.attention_layernorm.weight'] = model_params[
                'encoder.blocks.' + str(i) + '.self_attn_layer_norm.weight'].contiguous()
        weights[f'{trtllm_layer_name_prefix}.attention_layernorm.bias'] = model_params[
            'encoder.blocks.' + str(i) + '.self_attn_layer_norm.bias'].contiguous()

        t = torch.cat([
            model_params['encoder.blocks.' + str(i) + '.self_attn.q_proj.weight'],
            model_params['encoder.blocks.' + str(i) + '.self_attn.k_proj.weight'],
            model_params['encoder.blocks.' + str(i) + '.self_attn.v_proj.weight']
        ],
                      dim=0).contiguous()
        ...
        bias_shape = model_params['encoder.blocks.' + str(i) +
                                  '.self_attn.q_proj.bias'].shape
        dtype = model_params['encoder.blocks.' + str(i) +
                             '.self_attn.q_proj.bias'].dtype
        fused_bias = torch.cat([
            model_params['encoder.blocks.' + str(i) + '.self_attn.q_proj.bias'],
            torch.zeros([*bias_shape], dtype=dtype),
            model_params['encoder.blocks.' + str(i) + '.self_attn.v_proj.bias']
        ],
                               dim=0).contiguous()
        ...
        t = model_params['encoder.blocks.' + str(i) +
                         '.self_attn.out_proj.weight'].contiguous()

        weights[f'{trtllm_layer_name_prefix}.attention.dense.weight'] = t
        weights[f'{trtllm_layer_name_prefix}.attention.dense.bias'] = model_params[
            'encoder.blocks.' + str(i) + '.self_attn.out_proj.bias'].contiguous()

        weights[f'{trtllm_layer_name_prefix}.mlp_layernorm.weight'] = model_params[
            'encoder.blocks.' + str(i) + '.final_layer_norm.weight'].contiguous()
        weights[f'{trtllm_layer_name_prefix}.mlp_layernorm.bias'] = model_params[
            'encoder.blocks.' + str(i) + '.final_layer_norm.bias'].contiguous()

        t = model_params['encoder.blocks.' + str(i) +
                         '.fc1.weight'].contiguous()
        weights[f'{trtllm_layer_name_prefix}.mlp.fc.weight'] = t

        weights[f'{trtllm_layer_name_prefix}.mlp.fc.bias'] = model_params[
            'encoder.blocks.' + str(i) + '.fc1.bias'].contiguous()

        t = model_params['encoder.blocks.' + str(i) +
                         '.fc2.weight'].contiguous()
        weights[f'{trtllm_layer_name_prefix}.mlp.proj.weight'] = t

        weights[f'{trtllm_layer_name_prefix}.mlp.proj.bias'] = model_params[
            'encoder.blocks.' + str(i) + '.fc2.bias'].contiguous()

    weights['transformer.ln_f.weight'] = model_params['encoder.layer_norm.weight'].contiguous()
    weights['transformer.ln_f.bias'] = model_params['encoder.layer_norm.bias'].contiguous()

    return weight_only_quantize_dict(weights, quant_algo=quant_algo, plugin=True)


def convert_hf_whisper_decoder(model_metadata: dict, model_params: dict, quant_algo: str = None):
    weights = {}

    weights['transformer.vocab_embedding.weight'] = model_params[
        'decoder.embed_tokens.weight']
    weights['lm_head.weight'] = model_params[
        'decoder.embed_tokens.weight'].clone()
    weights['transformer.position_embedding.weight'] = model_params[
    ...
    for i in range(model_metadata['n_text_layer']):
        ...
        t = torch.cat([
            model_params['decoder.blocks.' + str(i) + '.self_attn.q_proj.weight'],
            model_params['decoder.blocks.' + str(i) + '.self_attn.k_proj.weight'],
            model_params['decoder.blocks.' + str(i) + '.self_attn.v_proj.weight']
        ],
                      dim=0)
        dst = weights[f'{trtllm_layer_name_prefix}.self_attention.qkv.weight'] = t
        ...
        weights[f'{trtllm_layer_name_prefix}.self_attention.qkv.bias'] = torch.cat(
            [
                model_params['decoder.blocks.' + str(i) + '.self_attn.q_proj.bias'],
                torch.zeros([*bias_shape], dtype=dtype),
                model_params['decoder.blocks.' + str(i) + '.self_attn.v_proj.bias']
            ],
            dim=0)
        weights[f'{trtllm_layer_name_prefix}.self_attention.dense.bias'] = model_params[
            'decoder.blocks.' + str(i) + '.self_attn.out_proj.bias']

        weights[
            f'{trtllm_layer_name_prefix}.self_attention_layernorm.weight'] = model_params[
                'decoder.blocks.' + str(i) + '.self_attn_layer_norm.weight']
        weights[
            f'{trtllm_layer_name_prefix}.self_attention_layernorm.bias'] = model_params[
                'decoder.blocks.' + str(i) + '.self_attn_layer_norm.bias']



        t = torch.cat([
            model_params['decoder.blocks.' + str(i) + '.encoder_attn.q_proj.weight'],
            model_params['decoder.blocks.' + str(i) + '.encoder_attn.k_proj.weight'],
            model_params['decoder.blocks.' + str(i) + '.encoder_attn.v_proj.weight']
        ],
                      dim=0)
        dst = weights[f'{trtllm_layer_name_prefix}.cross_attention.qkv.weight'] = t

        t = model_params['decoder.blocks.' + str(i) +
                         '.encoder_attn.out_proj.weight'].contiguous()
        dst = weights[f'{trtllm_layer_name_prefix}.cross_attention.dense.weight'] = t

        bias_shape = model_params['decoder.blocks.' + str(i) +
                                  '.encoder_attn.q_proj.bias'].shape
        dtype = model_params['decoder.blocks.' + str(i) +
                             '.encoder_attn.q_proj.bias'].dtype
        cross_attn_qkv_bias = torch.cat([
            model_params['decoder.blocks.' + str(i) + '.encoder_attn.q_proj.bias'],
            torch.zeros([*bias_shape], dtype=dtype),
            model_params['decoder.blocks.' + str(i) + '.encoder_attn.v_proj.bias']
        ],
                                        dim=0)
        ...
        weights[
            f'{trtllm_layer_name_prefix}.cross_attention.dense.bias'] = model_params[
                'decoder.blocks.' + str(i) + '.encoder_attn.out_proj.bias']

        weights[
            f'{trtllm_layer_name_prefix}.cross_attention_layernorm.weight'] = model_params[
                'decoder.blocks.' + str(i) + '.encoder_attn_layer_norm.weight']
        weights[
            f'{trtllm_layer_name_prefix}.cross_attention_layernorm.bias'] = model_params[
                'decoder.blocks.' + str(i) + '.encoder_attn_layer_norm.bias']
        ...
        weights[f'{trtllm_layer_name_prefix}.mlp.fc.bias'] = model_params[
            'decoder.blocks.' + str(i) + '.fc1.bias']
        weights[f'{trtllm_layer_name_prefix}.mlp.proj.bias'] = model_params[
            'decoder.blocks.' + str(i) + '.fc2.bias']

        weights[f'{trtllm_layer_name_prefix}.mlp_layernorm.weight'] = model_params[
            'decoder.blocks.' + str(i) + '.final_layer_norm.weight']
        weights[f'{trtllm_layer_name_prefix}.mlp_layernorm.bias'] = model_params[
            'decoder.blocks.' + str(i) + '.final_layer_norm.bias']

    weights['transformer.ln_f.weight'] = model_params['decoder.layer_norm.weight']
    weights['transformer.ln_f.bias'] = model_params['decoder.layer_norm.bias']
...
if __name__ == '__main__':
    ...
    if not os.path.exists(model_path):
        ...
        if not os.path.exists(model_path):
            model_path = os.path.join(args.model_dir, 'original_model.pt')
        if not os.path.exists(model_path):
    ...
    if not isinstance(model, dict):
        if hasattr(model, 'dims'):
            model = {
                'dims' : model.dims.__dict__, 'model_state_dict' : model.state_dict()
            }
    ...
    elif 'dims' not in model:
        with open(os.path.join(args.model_dir, 'config.json'), 'r') as file:
            model_metadata = file.read()

        model_metadata = json.loads(model_metadata)
        model_metadata = {
            'n_mels'    : model_metadata['num_mel_bins'],
            'n_vocab'   : model_metadata['vocab_size'],
            
            'n_audio_state' : model_metadata['d_model'],
            'n_audio_ctx'   : model_metadata['max_source_positions'],
            'n_audio_layer' : model_metadata['encoder_layers'],
            'n_audio_head'  : model_metadata['encoder_attention_heads'],

            'n_text_state' : model_metadata['d_model'],
            'n_text_ctx'   : model_metadata['max_target_positions'],
            'n_text_layer' : model_metadata['decoder_layers'],
            'n_text_head'  : model_metadata['decoder_attention_heads']

        }
        model = {
            'model_state_dict' : model, 'dims' : model_metadata
        }

    is_hf_model = False
    model_metadata = model['dims']
    model_state_dict = model['model_state_dict']
    if any(k.startswith('model') for k in model_state_dict):
        is_hf_model = True
        model_state_dict = {
            k[6:].replace('layers.', 'blocks.') : v
    ...
    def convert_and_save(component: str = "encoder"):
        ...
        if component == "encoder":
            if not is_hf_model:
                weights = convert_openai_whisper_encoder(model_metadata, model_state_dict, quant_algo=quant_algo)
            else:
                weights = convert_hf_whisper_encoder(model_metadata, model_state_dict, quant_algo=quant_algo)
        else:
            assert component == "decoder"
            if not is_hf_model:
                weights = convert_openai_whisper_decoder(model_metadata, model_state_dict, quant_algo=quant_algo)
            else:
                weights = convert_hf_whisper_decoder(model_metadata, model_state_dict, quant_algo=quant_algo)
