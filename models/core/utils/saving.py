...
def set_saving_dir(directory):
    """ Overrides the root directory where models are saved / restored (see `get_saving_dir`).

        Returns the *previous* value, so callers can restore it (e.g. tests isolating their I/O
        in a temporary directory). Prefer the `saving_dir` context manager when the override is
        meant to be scoped.
    """
    global _pretrained_models_dir
    previous = _pretrained_models_dir
    _pretrained_models_dir = directory
    return previous

@contextmanager
def saving_dir(directory):
    """ Scoped version of `set_saving_dir` : restores the previous root on exit (even on error). """
    previous = set_saving_dir(directory)
    try:
        yield directory
    finally:
...
def remove_training_checkpoint(name):
    ...
    if not os.path.exists(training_ckpt_dir): return
